...
class BeanParameterExtractor implements ParameterExtractor {
	...
	static BeanParameterExtractor create(Class<?> beanClass) {
		...
		Class<?>[] types = Arrays.stream(readMethods).map(Method::getReturnType)
				.toArray(Class<?>[]::new);
	...
	private static Method[] readMethods(Class<?> beanClass, String[] properties) {
		try {
			...
			return Arrays.stream(properties).map(collect::get).toArray(Method[]::new);
		}
	...
	public Object[] values(Object[] args) {
		Object bean = args[0];
		return Arrays.stream(readMethods).map(a -> readValue(bean, a)).toArray();
	}
